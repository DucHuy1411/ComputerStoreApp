...
import styles, { COLORS } from "../styles/RecentlyViewedStyle";
...
const normalizeRow = (x) => ({
  ...
  sku: String(x?.sku ?? ""),
  viewedAt: Number(x?.viewedAt ?? x?.time ?? 0),
});
        ...
        <ScrollView contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}>
          {items.length === 0 ? (
            <View style={styles.empty}>
              <Ionicons name="time-outline" size={42} color={COLORS.MUTED_ICON} />
              <Text style={styles.emptyTitle}>Chưa có lịch sử</Text>
              <Text style={styles.emptySub}>Mở một sản phẩm để lưu lại</Text>
            </View>
          ) : (
            items.map((it) => (
              <Pressable key={it.id} style={styles.row} onPress={() => openDetail(it.id)}>
                ...
                <Pressable onPress={() => removeOne(it.id)} hitSlop={10} style={styles.iconBtn}>
                  <Ionicons name="close" size={18} color={COLORS.MUTED_ICON} />
                </Pressable>
              </Pressable>
            ))
          )}
