...
import { apiAddresses, apiAddressSetDefault, apiAddressDelete, apiCheckoutFromCart, apiBuyNow } from "../services/endpoints";
...
const normalizeAddresses = (res) => {
  ...
  return arr.map((a) => {
    ...
    const type = typeRaw.includes("work") || typeRaw.includes("office") ? "work" : typeRaw.includes("home") ? "home" : "pin";

    const isDefault =
      !!a.isDefault || !!a.is_default || !!a.default || (typeof a.isDefault === "number" ? a.isDefault === 1 : false);
...
export default function CheckoutScreen({ navigation, route }) {
  ...
  const shippingOptions = useMemo(
    () => [
      { id: "standard", title: "Giao hàng tiêu chuẩn", sub: "Dự kiến giao: 25 - 27 Tháng 12", price: "Miễn phí", free: true },
      { id: "fast", title: "Giao hàng nhanh (24h)", sub: "Dự kiến giao: 23 Tháng 12", price: "50.000đ", free: false },
      { id: "store", title: "Nhận tại cửa hàng", sub: "Sẵn sàng sau 2 giờ", price: "Miễn phí", free: true },
    ],
    []
  );
  ...
  const products = useMemo(() => {
    ...
    if (product) {
      ...
      return [{ id: String(product.id || "p1"), name, qty: q, price: formatVnd(priceNumber) }];
    }
    return [];
  ...
  const totalText = useMemo(() => {
    ...
    if (product) return formatVnd(Number(product.price || 0) * Number(qty || 1));
    return "0đ";
  }, [order, product, qty]);
  ...
  const loadAddresses = useCallback(async () => {
    try {
      ...
      if (fixed.length && !fixed.some((a) => a.isDefault)) fixed[0] = { ...fixed[0], isDefault: true };
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      const payload = { addressId: selectedAddress.id, shippingMethod: shipMethod };

      if (from === "cart") {
        await apiCheckoutFromCart(payload);
      } else if (product) {
        await apiBuyNow({ ...payload, productId: product.id, qty: Number(qty || 1) });
      } else {
        await apiCheckoutFromCart(payload);
      }
      ...
      navigation.navigate("OrderSuccess", {
        order: {
          ...
          statusKey: "done",
          createdAt: Date.now(),
          address: { name: selectedAddress?.name, phone: selectedAddress?.phone, address: selectedAddress?.address },
            ...
            <View key={s.id} style={styles.stepItem}>
              ...
              >
                {isDone ? (
                  ...
                  <Text style={[styles.stepNum, isActive ? { color: COLORS.WHITE } : { color: COLORS.MUTED }]}>{s.id}</Text>
                )}
              </View>
              <Text style={[styles.stepLabel, isActive && { color: COLORS.BLUE }]}>{s.label}</Text>
            </View>
          );
        })}
      </View>
  ...
  const ShippingCard = ({ opt }) => {
    ...
    return (
      <Pressable style={[styles.shipItem, selected && styles.shipItemActive]} onPress={() => setShipMethod(opt.id)}>
        <Ionicons name={selected ? "radio-button-on" : "radio-button-off"} size={22} color={selected ? COLORS.BLUE : COLORS.FUTURE_BORDER} />
        <View style={{ flex: 1 }}>
          <Text style={styles.shipTitle}>{opt.title}</Text>
          <Text style={styles.shipSub}>{opt.sub}</Text>
          <Text style={[styles.shipPrice, opt.free ? { color: COLORS.GREEN } : { color: COLORS.BLUE }]}>{opt.price}</Text>
        </View>
      </Pressable>
    );
  };
  ...
  const AddressTypeIcon = ({ type }) => {
    const bg = type === "home" ? COLORS.BLUE : type === "work" ? "#F59E0B" : "#A855F7";
    const icon = type === "home" ? "home-outline" : type === "work" ? "briefcase-outline" : "location-outline";
  ...
  const AddressItem = ({ a }) => {
    ...
    return (
      <View style={[styles.addrCard, selected && styles.addrCardActive, busy && { opacity: 0.7 }]}>
        <Pressable style={styles.addrTopRow} onPress={() => (busy ? null : setDefaultAddress(a.id))}>
          <Ionicons name={selected ? "radio-button-on" : "radio-button-off"} size={20} color={selected ? COLORS.BLUE : COLORS.FUTURE_BORDER} />
        ...
        <View style={styles.addrActions}>
          <Pressable style={[styles.actionBtn, styles.actionDelete]} onPress={() => (busy ? null : deleteAddress(a.id))}>
            <Ionicons name="trash-outline" size={16} color={COLORS.RED} />
            <Text style={[styles.actionText, { color: COLORS.RED }]}>Xóa</Text>
          </Pressable>

          {!selected ? (
            <Pressable style={[styles.actionBtn, styles.actionDefault]} onPress={() => (busy ? null : setDefaultAddress(a.id))}>
              <Ionicons name="checkmark-circle-outline" size={16} color={COLORS.BLUE} />
              <Text style={[styles.actionText, { color: COLORS.BLUE }]}>Chọn</Text>
            </Pressable>
          ) : (
            <View style={{ flex: 1 }} />
          )}
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
          ...
          {addrLoading ? (
            ...
            <View style={styles.addrRow}>
              <View style={styles.addrIcon}>
                <Ionicons
                  name={selectedAddress.type === "home" ? "home-outline" : selectedAddress.type === "work" ? "briefcase-outline" : "location-outline"}
                  size={20}
                  color={COLORS.WHITE}
                />
              </View>
              <View style={{ flex: 1 }}>
                ...
                {!!selectedAddress.phone && <Text style={styles.addrPhone}>{selectedAddress.phone}</Text>}
        ...
        <View style={styles.card}>
          <Pressable style={styles.prodHeader} onPress={() => setProductsOpen((v) => !v)}>
            <View style={styles.cardHeaderLeft}>
            ...
            <View style={styles.prodRight}>
              ...
              <Ionicons name={productsOpen ? "chevron-up" : "chevron-down"} size={18} color={COLORS.MUTED} />
            </View>
          </Pressable>
      ...
      <View style={styles.bottomBar}>
        ...
        <Pressable style={[styles.orderBtn, !canPlace && styles.orderBtnDisabled]} onPress={placeOrder} disabled={!canPlace}>
          {placing ? <ActivityIndicator color={COLORS.WHITE} /> : <Text style={styles.orderBtnText}>Đặt hàng</Text>}
        </Pressable>
      </View>

      <Modal visible={addrModal} transparent animationType="fade" onRequestClose={() => setAddrModal(false)}>
        <Pressable style={styles.modalOverlay} onPress={() => setAddrModal(false)}>
          <Pressable style={styles.modalSheet} onPress={() => {}}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Chọn địa chỉ</Text>
              <Pressable onPress={() => setAddrModal(false)} style={styles.modalClose}>
